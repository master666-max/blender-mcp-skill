...
[![Version](https://img.shields.io/badge/version-2.8.2-blue)](https://github.com/master666-max/blender-mcp-skill/releases)
...
**8. 经验区 + 演化引擎（六档可调）**——包内 `experience/` 不只是笔记，它内置一枚**演化引擎**（evo-seat 微型内核：单文件、治理×演化融合、随包 vendored 逐字节未改）。任务实战经验以 EXP 条目（**证据三件套**：产物锚 + 输出原话 + 可复算命令）进入活区账本，引擎按**六档审查精细度**供给治理，装多少由你定：

| 档 | 装什么 |
|---|---|
| G0 观察位 | 账本（触发器物理 append-only）+ 检索 + 三态 |
| G1 标准位 | ＋ 链重放验证 + 墓碑 + 开库即验（新建库默认） |
| G2 类型位 | ＋ 条目构造校验 + 类型差异化衰减 |
| G3 门位 | ＋ 质量门五道（**经验区建议档**，随包桥首次入账自动升到此档） |
| G4 决策位 | ＋ 决策留痕 + 金标挣得 + defer 监控 |
| G5 锚定位 | ＋ 入库锚 + 出站扫描三态 + 周巡检 |

升级一条命令、单调升档、降级留痕；账本是**运行态**（不入发行包），首次使用初始化一次即可。**引擎只能写草案条目——晋升进正式手册仍必须过人 + 发版门**：自动化不放宽任何治理不变量（账本只增不改 / 出处三件套 / fail-closed / 墓碑不删）。
...
| `experience/` | **经验区**：活区账本（EXP 条目，证据三件套）+ **演化引擎**（evo-seat 微型内核：六档 G0-G5，经验区建议 G3；账本为运行态不入包；`exp_bridge.py` 桥接）——晋升进分册走发版门 |
| `docs/` | 设计文档与实战案例 |
| `showcase/` | 门面展示图 |
