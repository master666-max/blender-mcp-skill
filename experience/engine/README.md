...
> **来源**：审计区设计并实现——**打包目录 `<审计区目录>/evo-seat/`（真实路径见包外 HANDOFF——F-172 脱敏）**
> （原根级单文件已迁入该目录；架构书随迁 `evo-seat/docs/架构书-微内核-20260923.md`），
> `evo_seat.py` **v0.2.2**（§7 投影语义对齐 + **账本事务化**；SPEC 对齐/版本纪律/判别力修复承前），
> vendored **逐字节未改**。
> sha256 = `512f13b869528bf7ec5717178c814e5745621a078ab4fade82a4db2fd9479470`
> （升级时替换此文件并更新本行哈希 + 指纹 #92；配套映射见下，桥接脚本 `exp_bridge.py`）。
> 上一版 = `096077ce…`（v2.8.3 换装 v0.2.1；更早 `403d79a4…`/`43f8db8f…`/`61a808be…`，
> 见 CHANGELOG）。
>
> **v2.9.2 换装（上游 v0.2.2）**：① §7 投影语义对齐（与重装形态 evocore.project 同语义——
> 建条盖账本 ts / 命中回放 last_used / 墓碑生效）；② **账本事务化并入机制段**（`append`
> 取尾→INSERT 包进 `BEGIN IMMEDIATE`/COMMIT/ROLLBACK + `busy_timeout=5000`——七阶段
> 审计流水线 R1/R2 修复进原版；并发 4 写者×20 条冒烟 0 错、80 事件链验通过）。
> **换装证据**：selftest 11 测 + conformance 14/14 + 新库/旧账本双向兼容（28 事件链完整）。
> **新基线：`framework_sha = 622110a5af2cc4d7`（框架段 15,875 B，独立复算一致）**。
> **变更记录失准如实入档**：上游 0.2.2 变更记录称「机制段 §1-§6 未动 ⇒ framework_sha
> 不变」——实测 §1-§6 **+499 B**（事务化并入）、哈希已变；漏记被段哈希对账当场检出
> （0.2.1 判别力修复的直接收益），已回报上游。
>
...
> **新基线（v0.2.1 时点）：`framework_sha = a98f559753d7cbe0`（框架段长 13,297 B；
> v0.2.2 起以本 README 首部 v2.9.2 条为准）**。
> 对账口径（采纳上游回执 §四）：**段哈希为主**（区分「框架升级」与「宿主段改动」）
> + **整文件 sha256 为投递校验**（本 README 首部登记值 + 指纹 #92）。
>
> **残留守卫洞（本包回执核对实测；已回报上游——v0.2.2 复测仍在，待后续版本）**：① 内嵌判别力测试为
